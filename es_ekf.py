...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    #K_k = np.zeros((9, 3))
    R_sensor = sensor_var*np.eye(3) # assume it Isotropic
    K_k = p_cov_check @ h_jac.T @ np.linalg.pinv(h_jac @ p_cov_check @ h_jac.T + R_sensor)
    ...
    p_hat = p_check + e_p
    v_hat = v_check + e_v
    q_hat = Quaternion(axis_angle = e_q).quat_mult_left(q_check, out = 'Quaternion')
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    Q_k = np.eye(6)
    Q_k[:3,:3] = delta_t**2*var_imu_f*np.eye(3)
    Q_k[3:,3:] = delta_t**2*var_imu_w*np.eye(3)
    ...
    for i in range(len(gnss.t)):
        if abs(gnss.t[i] - imu_f.t[k]) < 0.01:
            p_check, v_check, q_check, p_cov_check = measurement_update(var_gnss, p_cov_check,
                                                    gnss.data[i], p_check, v_check, q_check)
    for i in range(len(lidar.t)):
        if abs(lidar.t[i] - imu_f.t[k]) < 0.01:
            p_check, v_check, q_check, p_cov_check = measurement_update(var_lidar, p_cov_check,
                                                    lidar.data[i], p_check, v_check, q_check)
